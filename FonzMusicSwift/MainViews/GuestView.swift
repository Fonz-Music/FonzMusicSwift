...
// ---------------------------------- created inside view -------------------------------------------
    ...
    var body: some View {
        ...
        VStack {
            GuestContainerView(guestPage: [0, 1], track: self.guestTrack, coaster: self.hostCoasterDetails, guestPagesUpdateDirection, queueAmount: $queuesUsed, currentGuestPageIndex: $GuestPageActive).environmentObject(guestPagesUpdateDirection)
        }.ignoresSafeArea()
    }
}
...
struct GuestContainerView: View {
    ...
    init( guestPage: [Int], track: GlobalTrack, coaster: HostCoasterInfo, _ newPage: UpdatePageViewVariables, queueAmount: Binding<Int>, currentGuestPageIndex: Binding<Int>) {
        
        self._currentGuestPageIndex = currentGuestPageIndex

        self.guestControllers = guestPage.map {UIHostingController(rootView: GuestPages(guestPage: $0, currentSong: track, coasterInfo: coaster, updatePageVars: newPage, currentGuestPageIndex: currentGuestPageIndex, queuesUsed: queueAmount))}
    }
