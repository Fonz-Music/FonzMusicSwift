...
//                            returnMessage = decodedResponse!.message
    ...
    func getTracksByPlaylistPaginated(sessionId: String, playlistId: String, offset: Int) -> [TrackForPagination] {
        // this allows us to wait before returning value
        let sem = DispatchSemaphore.init(value: 0)
        // return
        var tracks : [TrackForPagination] = [TrackForPagination]()
...
//                returnCode = response?.getStatusCode() ?? 0
                ...
                if let decodedResponse = try? JSONDecoder().decode(ItemsFromPlaylist.self, from: dataResp) {
                    ...
                    print("success")
//                    print("decoded resp is \(decodedResponse)")
                    tracks  = playlistTracksToTracksPaginated(playlistResps: decodedResponse, offset: offset)
